...
- 暂不支持多实例、多 Socket、用户级 printf 调试流、日志批量上传和连续系统变量步长仿真。
...
- `step_index`：仿真步编号，固定从 0 开始。Simulink 端每成功完成一次 `INPUT_DATA → OUTPUT_DATA` 交互后加 1；DSP 端维护 `expected_step_index`，`SIM_START` 后重置为 0，收到不等于期望值的 `INPUT_DATA` 时返回 `RESPONSE(error_code=7)` 并断开。DSP 端不得修改该值，只能在 `OUTPUT_DATA` 中原样返回。
- `input_data`：按配置文件定义顺序编码。

V2.3 不支持 `step_index` 回绕。Simulink 端在发送 `UINT32_MAX` 后如仍需继续仿真，必须报错终止，而不是回绕到 0。
...
7. 发送成功后将 `expected_step_index` 加 1。
...
DSP 端使用非阻塞状态机，不在 `C2837xBlock_Run()` 中长期阻塞。每次调用 `C2837xBlock_Run()` 只处理当前可用事件，然后立即返回。
...
| `mdlInitializeSampleTimes` | 设置固定离散采样时间或继承采样时间并在运行前校验 |
...
若采用 inherited sample time，则必须在初始化或运行前校验当前模型为 fixed-step discrete，且该模块不存在连续状态。V2.3 推荐直接由 App 生成显式 `C2837X_BLOCK_SAMPLE_TIME_SEC`，并将该值写入 `config_hash` 的 `sample_time_sec` 字段。
...
recv_frame()
    OUTPUT_DATA: 校验 length 与 step_index，unpack，写输出端口，检查溢出后 step_index++
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    ...
    if (type == C2837X_MSG_OUTPUT_DATA) {
        ...
        if (ctx->step_index == UINT32_MAX) {
            ssSetErrorStatus(S, "C2837xBlock: step_index overflow");
            return;
        }
        ctx->step_index++;
    }
...
C2837xBlock_Run()
    ...
    │         unpack input
    │         调用 OnStep
    │         如果返回0：pack output，发送 OUTPUT_DATA(step_index)，expected_step_index++
    │         如果返回非0：发送 RESPONSE(5)，进入 DISCONNECTED
    │     如果 SIM_STOP:
    │         调用 OnSimStop
    │         进入 DISCONNECTED
...
如果用户违反限制，S-Function 应在启动阶段报错，而不是静默运行。
...
6. 搭建 Simulink fixed-step discrete 测试模型。
7. 验证 1000 步以上连续通信。
...
- [ ] Fixed-step discrete 模型中通信步数与 sample hit 一致。
- [ ] variable-step solver 下模块明确报错或禁止使用。
...
- [ ] `step_index` 达到 `UINT32_MAX` 后继续仿真时 C S-Function 能终止仿真并报告溢出。
