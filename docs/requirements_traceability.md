...
本次保留以下边界：DSP/CCS target build、真实 COM、真实 Simulink 通信仍为 `NOT_EXECUTED`；半双工硬件、multi-SCI、完整 Baud/GPIO 矩阵和长期稳定性也不执行。用户明确提供的代表性结果为 `SINGLE_SCI_HARDWARE=PASS`、`SINGLE_W5300_HARDWARE=PASS`、`SCI_W5300_MIXED_HARDWARE=PASS`（1 SCI + 1 W5300）；它们不扩展为全硬件矩阵或最终 LSPCLK 寄存器确认。SCI-S5-02 产生的 MinGW64 8.1.0 代表性 SCI MEX 证据为 `REUSED PASS`；本次 S5-04 不重建 MEX，记为 `NOT_EXECUTED / NOT_REQUIRED`。
...
| --- | --- | --- | --- | --- | --- | --- |
| FR-001 | SCI 与 W5300 并存；支持 SCI-A/B/C/D、不同 SCI 多实例/混合实例，DSP 主循环轮询。 | `app/c2837x_block_iodevice_sci_definition.m`<br>`dsp/src/c2837x_block_sci.c`<br>`dsp/src/c2837x_block.c` | SCI-G2/G3/G4 CLOSED；SCI-S5-01 REUSED EVIDENCE | `tests/app/test_sci_iodevice_validation.m`<br>`tests/dsp_host/sci_iodevice_test.c` | IMPLEMENTED / DSP_CCS_NOT_EXECUTED | 软件实现与 Host 证据已存在；用户提供的代表性单 SCI、单 W5300、1 SCI + 1 W5300 mixed hardware 均为 PASS；multi-SCI、目标 CCS 仍未执行。 |
...
| FR-006 | W5300 及其 hot path 不变；Run 不做完整静态校验，PC step 不做全量复制/堆分配，端口/元素/端序由生成期固定。 | `dsp/src/c2837x_block.c`<br>`dsp/src/c2837x_w5300_hal.c`<br>`dsp/src/c2837x_w5300_channel.c`<br>`simulink/c2837x_block_pc_socket.c`<br>`app/c2837x_block_render_dsp_instance_io_files.m` | SCI-S5-01 historical evidence baseline `15f1311`；SCI-S5-04 focused audit @ `631bf5616104fed49005d5d904fd547f9e60fdbc` | `tests/dsp_host/test_s2_07_core_protocol.m`<br>`tests/app/test_dsp_output_model.m`<br>`tests/dsp_host/w5300_hal_init_test.c` | REUSED_EVIDENCE_PASS | `631bf561` 只对 W5300 HAL 的 EMIF1 GPIO ownership/initialization 做 focused correction；changed-path audit 确认 Core Run、W5300 channel/runtime hot path、PC `pc_socket` hot path 和 V1 wire protocol 未改变。因此 FR-006 的 non-regression 结论基于实际 changed-path audit，而不是“整个 W5300 DSP source 未修改”；本次不机械重跑历史 V1 全套测试。 |
...
| FR-017 | W5300 固定资源仍由 platform reserved resources 管理，不放入 capability JSON。 | `app/c2837x_block_get_platform_reserved_resources.m`<br>`app/c2837x_block_iodevice_w5300_tcp_definition.m`<br>`dsp/src/c2837x_w5300_hal.c` | SCI-G1/G2 CLOSED；SCI-S5-04 traceability closure correction @ `631bf5616104fed49005d5d904fd547f9e60fdbc` | `tests/app/test_sci_iodevice_validation.m`<br>`tests/dsp_host/w5300_hal_init_test.c` | STATIC_AUDIT_PASS | `631bf561` 使 Platform Reserved Resources 与实际 W5300 HAL 使用的 EMIF1 GPIO 重新一致：不再保留 W5300 未使用的 GPIO，释放的 GPIO 可供 SCI 使用；App resource-validation fixture 与 W5300 HAL focused host fixture 已同步。 |
| FR-018 | 合并 capability、platform 和全部实例资源；检测重复 module、GPIO 冲突、自冲突、W5300 资源冲突、缺 endpoint 和派生 pinmux。 | `app/c2837x_block_validate_project.m`<br>`app/c2837x_block_validate_project_structure.m`<br>`app/c2837x_block_get_platform_reserved_resources.m`<br>`app/c2837x_block_iodevice_sci_definition.m`<br>`dsp/src/c2837x_w5300_hal.c` | SCI-G1/G2 CLOSED；SCI-S5-04 traceability closure correction @ `631bf5616104fed49005d5d904fd547f9e60fdbc` | `tests/app/test_sci_iodevice_validation.m`<br>`tests/app/test_project_validation.m`<br>`tests/dsp_host/w5300_hal_init_test.c` | REUSED_EVIDENCE_PASS | 当前 resource validation 继续合并 capability/platform/instance 资源；`631bf561` 同步 reserved-resource 期望值与 W5300 HAL 初始化事实，相关 App fixture 和 HAL focused host fixture 已复用/同步；不扩大为硬件资源矩阵。 |
...
| FR-035 | SCI 固定 8N1、无 parity/flow control、async/full-duplex；关闭 autobaud/loopback/interrupt，FIFO 开启、delay=0、polling。 | `dsp/src/c2837x_block_platform.c`<br>`dsp/src/c2837x_block_sci.c`<br>`app/c2837x_block_render_sfun_files.m` | SCI-G2/G3/G4 CLOSED | `tests/dsp_host/test_s2_03_sci.m`<br>`tests/dsp_host/sci_platform_init_test.c` | REUSED_EVIDENCE_PASS | Host/静态证据支持配置；用户提供的代表性单 SCI communication 在当前最终配置下为 PASS；本记录不扩展为示波器、全 pin/baud matrix 或寄存器实测结论。 |
...
| FR-037 | PlatformInit 按 descriptor 配置 RX/TX mux、pad、qualification 和可选 CTRL；不触碰未使用资源。 | `dsp/src/c2837x_block_platform.c`<br>`app/c2837x_block_iodevice_sci_definition.m` | SCI-G2 CLOSED | `tests/dsp_host/sci_platform_init_test.c`<br>`tests/dsp_host/platform_sci_modes_test.c` | STATIC_AUDIT_PASS | 本次未新增 CCS/read-back 证据；用户已提供当前最终配置下代表性单 SCI communication PASS，但未独立确认完整寄存器/GPIO matrix。 |
...
| FR-076 | 每实例 S-Function root 自包含；W5300 使用 pc_socket，SCI 使用 pc_serial，只包含当前 transport，不共享 PC runtime。 | `app/c2837x_block_build_sfun_output_model.m`<br>`app/c2837x_block_render_sfun_files.m`<br>`app/c2837x_block_render_sfun_build_files.m` | SCI-G3/G4 CLOSED；SCI-S5-01 REUSED EVIDENCE；SCI-S5-04 audit @ `631bf5616104fed49005d5d904fd547f9e60fdbc` | `tests/app/test_sfun_build_candidates.m`<br>`tests/app/test_sci_s4_04_transport_build_generation.m` | REUSED_EVIDENCE_PASS | 历史 S5-01 evidence baseline `0700f3ab390aeac396723cb016fdf616ea96058b` 仅作阶段证据；当前 `631bf561` 未改变 W5300 PC `pc_socket` hot path 或 S-Function transport 选择。 |
...
| FR-088 | 不要求多 SCI/多硬件 matrix；mixed 只做确定性生成证据。 | `app/c2837x_block_build_dsp_candidates.m`<br>`app/c2837x_block_build_project_candidates.m`<br>`app/c2837x_block_build_sfun_candidates.m` | SCI-S5-01 REUSED EVIDENCE；SCI-S5-04 | `tests/app/test_dsp_generation_flow.m`<br>`tests/app/test_sfun_build_candidates.m` | GOVERNANCE_PASS | 不建立多 SCI/完整硬件 matrix；用户另提供的代表性 1 SCI + 1 W5300 mixed hardware 为 PASS，确定性候选生成仍是开发侧边界内证据。 |
| FR-089 | 最终 SCI 硬件、用户 LSPCLK/baud/CTRL/mixed/稳定性由用户验证；无真实硬件不得写 PASS。 | `app/c2837x_block_get_sci_clock_config.m`<br>`app/c2837x_block_render_dsp_project_files.m`<br>`docs/ccs_integration_and_dual_instance_main.md` | SCI-S5-02 CLOSED；SCI-S5-03；SCI-S5-04 traceability closure correction | `docs/simulink_mex_user_guide.md`<br>`docs/ccs_integration_and_dual_instance_main.md`<br>User-provided hardware validation in SCI-S5-04 closure input | IMPLEMENTED / USER_VALIDATION_PENDING | 已完成并记录：代表性单 SCI hardware = PASS、单 W5300 hardware = PASS、1 SCI + 1 W5300 mixed hardware = PASS；仍未建立 multi-SCI、完整 Baud/GPIO matrix、half-duplex hardware、长期稳定性或 explicit final LSPCLK register/hardware confirmation。 |
| FR-090 | CCS/MEX/Simulink/hardware 未执行项必须显式标记，不能隐含为 PASS。 | `README.md`<br>`docs/ccs_integration_and_dual_instance_main.md`<br>`docs/simulink_mex_user_guide.md` | SCI-S5-03 CLOSED；SCI-S5-04 traceability closure correction | `docs/simulink_mex_user_guide.md`<br>`plan.md`<br>User-provided hardware validation in SCI-S5-04 closure input | GOVERNANCE_PASS | 未执行/待确认项逐项保留真实状态；三项代表性硬件结果按用户提供的范围记录为 PASS，未推断为多 SCI、全 Baud/GPIO、half-duplex、LSPCLK 寄存器或稳定性 PASS。 |
...
- Protocol/W5300 implementation audit：`REUSED AUDIT EVIDENCE`；当前 correction 限定为 HAL initialization/resource ownership。
...
- `SCI_W5300_MIXED_HARDWARE = PASS`
- `MULTI_SCI_MATRIX = NOT_EXECUTED / NOT_REQUIRED`
- `HALF_DUPLEX_HARDWARE = NOT_EXECUTED`
...
- SCI-S5-04 closure input：用户明确提供的代表性单 SCI、单 W5300、1 SCI + 1 W5300 mixed hardware 结果均为 PASS；这些是用户真实验证结果，本任务不重新执行或扩展其范围。
...
- `SINGLE_SCI_HARDWARE = PASS`：当前最终 SCI 平台配置下的代表性 SCI hardware communication。
- `SINGLE_W5300_HARDWARE = PASS`：代表性单 W5300 hardware。
- `SCI_W5300_MIXED_HARDWARE = PASS`：代表性 1 SCI + 1 W5300 mixed hardware。
- 上述结果只证明用户明确提供的代表性场景；不推断 SCI-A/B/C/D 全部通过、五个 Baud、全部 RX/TX GPIO、CTRL/half-duplex、多 SCI、完整 mixed 组合、长期稳定性或 final LSPCLK 寄存器值。
...
- Explicit final LSPCLK register/hardware confirmation：`USER_VALIDATION_PENDING`。
- Half-duplex hardware：`NOT_EXECUTED`。
- Multi-SCI matrix：`NOT_EXECUTED / NOT_REQUIRED`。
- Full Baud/GPIO matrix：`NOT_EXECUTED / NOT_REQUIRED`。
- Long stability：`NOT_EXECUTED / NOT_REQUIRED`。
- SCI-S5-04 product tests and MEX build：`NOT_EXECUTED / NOT_REQUIRED`。

### USER_VALIDATION_PENDING

- Explicit final LSPCLK register/hardware confirmation；未覆盖的 Baud/BRR、GPIO/CTRL combinations、half-duplex、multi-SCI、完整 mixed combinations 和长期稳定性。
