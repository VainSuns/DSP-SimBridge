...
zeroBranch = regexp(source, ...
    'if\s*\(sent_bytes\s*==\s*0\)(?<body>[\s\S]*?return\s+0\s*;\s*\})', ...
    'names', 'once');
verifyNotEmpty(testCase, zeroBranch);
verifyEmpty(testCase, regexp(zeroBranch.body, ...
    'tx_sent_bytes\s*\+=|frame_start_tick\s*=|tx_done_action\s*=|expected_step_index\s*\+\+', ...
    'once'));
verifyNotEmpty(testCase, regexp(zeroBranch.body, 'return\s+0\s*;', 'once'));
end
...
verifyEmpty(testCase, regexp(sources, ...
    'dummy|Sn_MR_UDP|faulted\s*=\s*1|SIPR|SUBR|GAR|SHAR', 'once'));
end
