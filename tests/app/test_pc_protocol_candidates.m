classdef test_pc_protocol_candidates < matlab.unittest.TestCase
    ...
    methods (Test)
        ...
        function testBatch3ProtocolAndClockSourceContracts(testCase)
            protocolCandidate = testCase.Candidates( ...
                [testCase.Candidates.instance_index] == 1 & ...
                endsWith({testCase.Candidates.target_path}, 'axis_alpha_protocol.c'));
            ...
            testCase.verifyEmpty(regexp(protocol, ...
                'protocol_send_frame\s*\(', 'once'));
            testCase.verifySubstring(protocol, ...
                'axis_alpha_protocol_send_complete_frame');
            testCase.verifySubstring(protocol, ...
            ...
            nowStart = strfind(socket, 'static int axis_alpha_now_ms');
            clockStart = strfind(socket, 'static int axis_alpha_clock_error');
            testCase.verifyTrue(isscalar(nowStart) && isscalar(clockStart) && ...
                nowStart < clockStart);
            testCase.verifyEmpty(strfind( ...
                socket(nowStart:clockStart - 1), 'QueryPerformanceFrequency'));
            initStart = strfind(socket, ...
                'int axis_alpha_pc_socket_init(AxisAlphaPcSocket *socket');
