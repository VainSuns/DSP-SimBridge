...
testCase.verifyNumElements(dependencies, 29);
testCase.verifyEqual({dependencies(1:12).identity}, { ...
    ...
    'dsp-generator:crc32', ...
    'dsp-generator:iodevice-definition:w5300_tcp'});
testCase.verifyEqual({dependencies(1:12).role}, ...
    repmat({'generator_template'}, 1, 12));
testCase.verifyEqual({dependencies(13:end).role}, repmat({'core_source'}, 1, 17));
testCase.verifyEqual({dependencies.source_kind}, repmat({'file'}, 1, 29));
testCase.verifyEqual(numel(unique({dependencies.identity})), 29);
