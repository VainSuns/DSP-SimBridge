        ...
        function testInstanceInitializerUsesNamedEnum(testCase)
            ...
            testCase.verifyNotEmpty(regexp(header, ...
                ['C2837X_BLOCK_INSTANCE_INITIALIZER\(config_ptr\)' ...
                 '[\s\S]{0,100}C2837X_BLOCK_STATE_INERT'], ...
                'once'));
...
testCase.verifyNumElements(dependencies, 31);
testCase.verifyEqual({dependencies(1:14).identity}, { ...
    ...
    'dsp-generator:build-wire-layout', ...
    'dsp-generator:render-instance-io-files', ...
    'dsp-generator:render-algorithm-files', ...
    'dsp-generator:resolve-external-algorithm-source', ...
    'dsp-generator:build-instance-c-names', ...
    'dsp-generator:build-interface-hash', ...
    'dsp-generator:build-interface-text', ...
    'dsp-generator:crc32', ...
    'dsp-generator:iodevice-definition:w5300_tcp'});
testCase.verifyEqual({dependencies(1:14).role}, ...
    repmat({'generator_template'}, 1, 14));
testCase.verifyEqual({dependencies(15:end).role}, repmat({'core_source'}, 1, 17));
testCase.verifyEqual({dependencies.source_kind}, repmat({'file'}, 1, 31));
testCase.verifyEqual(numel(unique({dependencies.identity})), 31);
