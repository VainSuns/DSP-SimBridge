classdef test_configurator_layout < matlab.unittest.TestCase
    ...
    properties (TestParameter)
        issueRoute = struct( ...
            ...
            'sci', struct('code', 'SCI_RX_GPIO_REQUIRED', ...
                'field', 'project.instances(1).iodevice.settings.rx_gpio', ...
                'main', 'Instances', 'child', 'IoDevice'), ...
    ...
    methods (Test)
        ...
        function testNestedTabs(testCase)
            ...
            testCase.verifyEqual(nested_tab_titles(testCase.App.UIFigure, ...
                'Instances'), sort({'Algorithm', 'General', 'IoDevice'}));
        end
        ...
        function testIoDeviceControlContracts(testCase)
            iodevice = find_one_by_tag(testCase, testCase.App.UIFigure, ...
                'IoDeviceTypeField');
            module = find_one_by_tag(testCase, testCase.App.UIFigure, ...
                'SciModuleField');
            baud = find_one_by_tag(testCase, testCase.App.UIFigure, ...
                'SciBaudField');
            actualBaud = find_one_by_tag(testCase, testCase.App.UIFigure, ...
                'SciActualBaudField');
            udpSocket = find_one_by_tag(testCase, testCase.App.UIFigure, ...
                'W5300UdpSocketField');
            udpPort = find_one_by_tag(testCase, testCase.App.UIFigure, ...
                'W5300UdpPortField');
            ...
            testCase.verifyEqual(udpSocket.Items, cellstr(compose('%u', 0:7)));
            testCase.verifyEqual(udpPort.Value, 5000);
            udpLabels = findall(udpSocket.Parent, 'Type', 'uilabel');
            testCase.verifyEqual(sort({udpLabels.Text}), ...
                sort({'Socket Number', 'UDP Port'}));
            testCase.verifyEqual(module.Items, ...
                {'Not Selected', 'SCI-A', 'SCI-B', 'SCI-C', 'SCI-D'});
            testCase.verifyEqual(module.ItemsData, ...
        ...
        function testStableTags(testCase)
            tags = {'InstanceDetailTabGroup', 'InstanceGeneralTab', ...
                'InstanceIoDeviceTab', 'InstanceAlgorithmTab', ...
                'W5300SocketField', 'W5300TcpPortField', ...
                'W5300UdpSocketField', 'W5300UdpPortField', ...
                'SciActualBaudField', 'SciBaudErrorField', ...
                'SciRxGpioField', 'SciTxGpioField', 'SciCtrlGpioField', ...
                'ProjectSciClockField', 'GenerationPreviewInstanceContext'};
            counts = cellfun(@(tag) numel(findall( ...
                testCase.App.UIFigure, 'Tag', tag)), tags);
            testCase.verifyEqual(counts, ones(size(tags)));
            testCase.verifyEmpty(findall(testCase.App.UIFigure, ...
                'Tag', 'SciPinGroupField'));
        end
