classdef test_dsp_project_candidates < matlab.unittest.TestCase
    ...
    methods (Test)
        function testUserInstancesDriveDeterministicProjectFiles(testCase)
            ...
            verify_order(testCase, source, {'motor_control_config.h', ...
                'thermal_monitor_config.h', 'g_motor_control', 'g_thermal_monitor'});
            ...
            testCase.verifyEmpty(regexp(source, ...
                'extern\s+const\s+C2837xBlock_Config', 'once'));
        ...
        function testGeneratedProjectAndPublicMainCompileToObjects(testCase)
            ...
            for name = {'motor_control', 'thermal_monitor'}
                write_text(fullfile(generatedInc, [name{1} '_config.h']), sprintf([ ...
                    '#include "c2837x_block_config_internal.h"\n' ...
                    'extern const C2837xBlock_Config c2837x_block_%s_config;\n'], name{1}));
            end
            ...
            includeFlags = sprintf('-I"%s" -I"%s" -I"%s" -I"%s"', ...
