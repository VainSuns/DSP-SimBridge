classdef test_sfun_lifecycle_candidates < matlab.unittest.TestCase
    ...
    methods (Test)
        function testGeneratedLifecycleAgainstMockTcp(testCase)
            ...
            terminate = extractAfter(alphaSource, 'static void mdlTerminate');
            testCase.verifyTrue(contains(terminate, ...
                'if (ssGetPWork(S) == NULL) return;'));
            testCase.verifyLessThan(strfind(terminate, 'ssGetPWork(S)'), ...
                strfind(terminate, 'ssGetPWorkValue(S, 0)'));
