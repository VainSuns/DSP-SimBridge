classdef test_dsp_instance_config_candidates < matlab.unittest.TestCase
    ...
    methods (Test)
        function testCandidateCountsOrderAndAvailability(testCase)
            ...
            testCase.verifyNumElements(oneCandidates, 25);
            testCase.verifyNumElements(twoCandidates, 31);
            testCase.verifyNumElements(threeCandidates, 37);
        ...
        function testNamingHashSizesAndStaticBindings(testCase)
            ...
            testCase.verifyEmpty(strfind(axisHeader, ...
                'c2837x_block_config_internal.h'));
            testCase.verifyEmpty(regexp(axisHeader, ...
                'C2837xBlock_(Config|AlgorithmAdapter)', 'once'));
            testCase.verifyNotEmpty(strfind(axisHeader, ...
            ...
            testCase.verifyNotEmpty(strfind(axisSource, ...
...
for name = {'c2837x_block_internal.h', ...
        'c2837x_block_config_internal.h', 'c2837x_block_platform.h'}
    copyfile(fullfile(repositoryRoot, 'dsp', 'src', name{1}), ...
...
text = sprintf([ ...
    ...
    '''render_instance_io_support'',@(varargin) struct(''source_includes'',{{''x.h''}},''source_definitions'',''x''));\nend\n'], ...
