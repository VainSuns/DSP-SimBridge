...
│   │   ├── c2837x_block_algorithm.h # 用户算法接口
│   │   ├── c2837x_block_config.h   # DSP 配置（生成）
│   │   ├── c2837x_block_protocol.h # 协议定义
│   │   ├── c2837x_w5300_hal.h      # W5300 硬件抽象层
│   │   ├── c2837x_w5300_regs.h     # W5300 寄存器定义
│   │   └── c2837x_w5300_socket.h   # Socket 封装
│   └── src/                        # 源文件
│       ├── c2837x_block.c          # DSP 主实现
│       ├── c2837x_block_config.c   # 配置实现（生成）
│       ├── c2837x_block_global_variable.c # 全局变量（生成）
...
│   ├── c2837x_block_sfun_io.c      # 端口 I/O（生成）
│   ├── c2837x_block_pc_config.h    # PC 配置（生成）
...
- DSP 端配置文件（输出到指定的 DSP 目录）
- PC 端配置文件（输出到指定的 Simulink 目录）

### 步骤 4：编译 C S-Function
...
### MATLAB Level-2 S-Function（遗留）

`c2837x_block_sfun_matlab.m` 是阶段一的遗留验证工具，使用 MATLAB `tcpclient`。仅用于功能验证，不推荐用于正式仿真。
...
int C2837xBlock_OnStep(void)
{
    ...
    // 写入输出
    c2837x_block_output.sum = result;
...
| 版本 | 日期 | 说明 |
|------|------|------|
| V2.3 | 2024 | C S-Function、完整协议实现 |
| V2.2 | - | 协议规范完善 |
| V2.1 | - | MATLAB App 配置工具 |
| V1.0 | - | 初始版本，MATLAB S-Function |
