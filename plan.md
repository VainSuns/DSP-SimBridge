...
> **准入说明：** 阶段0可以开始；阶段1须先通过G0；阶段2及后续阶段按前序门禁依次进入。
---
## 1. 计划目标与执行规则
...
| 废弃 | 任务取消但编号保留 |
### 1.2 每个任务的最小交付信息
每个任务完成时必须记录：任务编号、对应FR、修改文件、验证结果、未验证范围和遗留问题。阶段汇总使用以下格式：
| 需求编号 | 实现文件 | 状态 | 验证方式 | 备注 |
|---|---|---|---|---|
---
## 2. 当前仓库改造基线
当前仓库是旧单实例实现，计划实施时重点处理以下现状：
...
- **输出文件：** `plan.md`中的任务表、阶段门禁和最终追踪表
- **实现目标：** 以后所有实施、审核和测试通过本计划任务编号及FR编号追踪，不增加单独的需求追踪文档。
- **非目标：** 不预填虚假的已完成状态。
...
- **输出文件：** 生成结果静态审核记录（保存在计划状态表/提交说明中）
...
- **前置任务：** S5-01～S5-04
- **输入文件：** 所有实现和文档、计划任务状态
- **输出文件：** `README.md`和本计划最终需求追踪表
- **实现目标：** README从旧单实例描述更新为实际多实例实现；填充FR实现文件、验证状态和未验证范围。
...
| 复审项 | 计划修改位置 | 处理结果 |
|---|---|---|
| R-01 close/Erratum软件超时 | S2-06、G2、风险表 | 已增加CPU Timer 2软件期限、全部无响应状态和faulted测试 |
| R-02 S2-08前置依赖 | S2-08 | 前置任务改为S2-02、S2-06、S2-07 |
| R-03统一测试基础设施 | 新增S0-05、G0、G1 | 已建立四类测试目录、统一入口和门禁记录格式责任任务 |
| R-04错误码符号和语义 | S0-02、G0、第13节 | 已增加0～8错误码数值、DSP/PC历史符号、固定线缆语义和产生策略矩阵 |
| R-05两级校验边界 | S1-10、G1 | 已区分即时校验与完整校验，UI仅触发服务 |
| R-06外部算法文件约束 | S1-05、S3-04、G1 | 已固定C源文件、存在、可读、普通文件、文件系统感知的`.c`扩展名及预览/提交复核 |
| R-07 8个Socket内存表述 | S2-02、G2 | 已明确每个Socket TX=8KB、RX=8KB及未使用Socket行为 |
| R-08旧Hash兼容入口 | S1-06 | 仅允许薄包装转发到新服务，禁止两套Hash并存 |
| R-09 last_error恢复条件 | S2-08、G2 | 仅Init或正常会话结束可恢复NONE，普通成功推进不得清除 |
| 最终R1 DSP/PC错误码符号矩阵 | S0-02、G0、第13节 | 已分别固定DSP与PC历史符号，宏名差异不再误判为协议冲突 |
| 最终R2 `.c`扩展名大小写 | S1-05、S1-10、S3-04、G1 | 已明确按当前平台文件系统实际规则判断，不使用无条件大小写转换 |
| 最终R3 S2-08输入依赖 | S2-08 | 输入文件已加入S2-06 |
| 最终审核状态 | 计划头、第14节、准入说明 | 已改为Approved for Implementation |
