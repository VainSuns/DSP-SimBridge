...
%C2837X_BLOCK_BUILD_SFUN_CANDIDATES Build deterministic S-Function candidates.
...
for index = 1:numel(model.files)
    ...
    switch suffix
        ...
        case [result.internal_name '_pc_udp.c']
            bytes = pcResult.udp_source_bytes;
        case [result.internal_name '_pc_udp.h']
            bytes = pcResult.udp_header_bytes;
        case [result.internal_name '_pc_serial.c']
            bytes = pcResult.serial_source_bytes;
        case [result.internal_name '_pc_serial.h']
            bytes = pcResult.serial_header_bytes;
...
dependencies = generator_dependencies(project);
issues = struct('severity', {}, 'code', {}, 'message', {}, ...
    'field_path', {}, 'instance_index', {}, 'file_path', {});
end

function dependencies = generator_dependencies(project)
...
types = cell(1, numel(project.instances));
for index = 1:numel(project.instances)
    types{index} = char(project.instances(index).iodevice.type);
end
if any(strcmp(types, 'sci'))
    files = [files, {fullfile('..', 'simulink', ...
        'c2837x_block_pc_serial.c'), ...
        fullfile('..', 'simulink', 'c2837x_block_pc_serial.h')}];
end
if any(strcmp(types, 'w5300_tcp'))
    files = [files, {fullfile('templates', 'pc_socket.c.in'), ...
        fullfile('templates', 'pc_socket.h.in')}];
end
if any(strcmp(types, 'w5300_udp'))
    files = [files, {fullfile('templates', 'pc_udp.c.in'), ...
        fullfile('templates', 'pc_udp.h.in')}];
end
files = [files, {fullfile('templates', 'protocol.c.in'), ...
    fullfile('templates', 'protocol.h.in'), ...
    'c2837x_block_build_instance_c_names.m'}];
if any(strcmp(types, 'sci'))
    files = [files, {'c2837x_block_get_sci_clock_config.m', ...
        'c2837x_block_calculate_sci_baud.m'}];
end
