function model = c2837x_block_build_sfun_output_model(project)
%C2837X_BLOCK_BUILD_SFUN_OUTPUT_MODEL Describe generated S-Function outputs.

c2837x_block_validate_project_structure(project);
sfunRoot = canonical_root(project.output.sfun_root);
commonSuffixes = {'sfun.c', 'sfun.h', 'sfun_io.c', 'sfun_config.h', ...
    'sfun_user_config.h', 'pc_error.h'};
commonCategories = {'auto_generated', 'auto_generated', 'auto_generated', ...
    'auto_generated', 'user', 'auto_generated'};
commonResponsibilities = { ...
    'Normal-mode C MEX S-Function callbacks and private context lifetime.', ...
    'Instance context, temporary outputs, and helper declarations.', ...
    'Port setup, INPUT_DATA packing, OUTPUT_DATA decoding, and output commit.', ...
    'Generated S-Function, wire layout, sample time, and port constants.', ...
    'User-editable PC connect, step, and terminate timeouts.', ...
    'Shared transport-neutral PC error contract without socket dependency.'};
tailSuffixes = {'protocol.c', 'protocol.h', 'build_sfun.m'};
tailCategories = {'auto_generated', 'auto_generated', 'auto_generated'};
tailResponsibilities = { ...
    'Instance-private V1 frame serialization and receive validation.', ...
    'Instance-private V1 wire constants and protocol API.', ...
    'Deterministic self-contained MEX build script.'};
prototype = struct('relative_path', '', 'target_path', '', 'category', '', ...
    'owner', '', 'instance_index', 0, 'file_scope', '', ...
    'responsibility', '', 'source_path', '', 'candidate_available', false);
files = repmat(prototype, 1, 11 * numel(project.instances));
fileIndex = 0;
for instanceIndex = 1:numel(project.instances)
    [transportSuffixes, transportCategories, transportResponsibilities] = ...
        transport_file_specs(project.instances(instanceIndex));
    suffixes = [commonSuffixes transportSuffixes tailSuffixes];
    categories = [commonCategories transportCategories tailCategories];
    responsibilities = [commonResponsibilities transportResponsibilities ...
        tailResponsibilities];
    names = c2837x_block_build_instance_c_names( ...
        project.instances(instanceIndex).internal_name);
    instanceRoot = c2837x_block_normalize_absolute_path( ...
        fullfile(sfunRoot, names.internal_name));
    for suffixIndex = 1:numel(suffixes)
        if strcmp(suffixes{suffixIndex}, 'build_sfun.m')
            fileName = ['build_' names.internal_name '_sfun.m'];
        else
            fileName = [names.internal_name '_' suffixes{suffixIndex}];
        end
        relativePath = [names.internal_name '/' fileName];
        targetPath = c2837x_block_normalize_absolute_path( ...
            fullfile(instanceRoot, fileName));
        if ~is_descendant(instanceRoot, targetPath)
            error('C2837xBlock:SfunOutput:PathEscape', ...
                'S-Function output path escapes its instance directory.');
        end
        fileIndex = fileIndex + 1;
        files(fileIndex) = struct('relative_path', relativePath, ...
            'target_path', targetPath, 'category', categories{suffixIndex}, ...
            'owner', ['sfun-instance:' names.internal_name ':' relativePath], ...
            'instance_index', double(instanceIndex), 'file_scope', 'instance', ...
            'responsibility', responsibilities{suffixIndex}, 'source_path', '', ...
            'candidate_available', true);
    end
end
paths = lower_cell({files.target_path});
if numel(unique(paths)) ~= numel(paths)
    error('C2837xBlock:SfunOutput:PathConflict', ...
        'S-Function output model contains duplicate or case-conflicting paths.');
end
model = struct('schema_version', uint16(1), 'sfun_root', sfunRoot, ...
    'files', files);
end

function [suffixes, categories, responsibilities] = transport_file_specs(instance)
switch char(instance.iodevice.type)
    case 'w5300_tcp'
        suffixes = {'pc_socket.c', 'pc_socket.h'};
        categories = {'auto_generated', 'auto_generated'};
        responsibilities = { ...
            'Instance-private cross-platform TCP implementation.', ...
            'Instance-private Socket context using the shared PC error API.'};
    case 'w5300_udp'
        suffixes = {'pc_udp.c', 'pc_udp.h'};
        categories = {'auto_generated', 'auto_generated'};
        responsibilities = { ...
            'Instance-private cross-platform UDP implementation.', ...
            'Instance-private UDP context using the shared PC error API.'};
    case 'sci'
        suffixes = {'pc_serial.c', 'pc_serial.h'};
        categories = {'auto_generated', 'auto_generated'};
        responsibilities = { ...
            'Instance-private production-derived Windows serial implementation.', ...
            'Instance-private production-derived serial API and error binding.'};
    otherwise
        error('C2837xBlock:SfunOutput:UnsupportedIoDevice', ...
            'S-Function output model does not support IoDevice "%s".', ...
            char(instance.iodevice.type));
end
end

function root = canonical_root(value)
if ~((ischar(value) && isrow(value)) || ...
        (isstring(value) && isscalar(value) && ~ismissing(value)))
    invalid_root();
end
value = char(value);
try
    root = c2837x_block_normalize_absolute_path(value);
catch
    invalid_root();
end
if isempty(root) || ~strcmp(value, root)
    invalid_root();
end
end

function invalid_root()
error('C2837xBlock:SfunOutput:InvalidRoot', ...
    'project.output.sfun_root must be canonical absolute text.');
end

function values = lower_cell(values)
values = cellfun(@lower, values, 'UniformOutput', false);
end

function tf = is_descendant(root, path)
prefix = [root filesep];
if ispc
    tf = startsWith(path, prefix, 'IgnoreCase', true);
else
    tf = startsWith(path, prefix);
end
end
