...
typedPrefixes = cell(1, numel(project.instances));
...
if strcmp(mode, 'full')
    ...
    function validate_instance(instance, index)
        ...
        if ~validNames(index)
            ...
            typedPrefixes{index} = names.typed_prefix;
        end
    ...
    function validate_payload(limit, inputOctets, outputOctets, prefix, index)
        ...
        if strcmp(char(project.instances(index).iodevice.type), 'w5300_udp') && ...
                double(limit) > 1468
            add('Error', 'UDP_MAX_PAYLOAD_TOO_LARGE', ...
                'UDP maximum payload must not exceed 1468 wire octets.', ...
                fieldPath, index, '');
        end
    ...
    function validate_unique_resources()
        ...
        for later = 2:numel(project.instances)
            for earlier = 1:later - 1
                if validNames(later) && validNames(earlier) && strcmp( ...
                        typedPrefixes{later}, typedPrefixes{earlier})
                    add('Error', 'GENERATED_C_NAME_CONFLICT', sprintf([ ...
                        'Internal names "%s" and "%s" generate the same ' ...
                        'strong-type and callback prefix "%s".'], ...
                        project.instances(earlier).internal_name, ...
                        project.instances(later).internal_name, ...
                        typedPrefixes{later}), ...
                        sprintf('project.instances(%u).internal_name', later), ...
                        later, '');
                    validNames(later) = false;
                end
            end
